...
public class Encrypt 
{
	...
	public static String get_SHA256(String data){	// 데이터값을 받아와 SHA256 암호화를 진행하는 메소드
		try{
	...
	public static String get_RIPEMD160(byte[] msg){	// 데이터값을 받아와 RIPEMD160 암호화를 진행하는 메소드
		...
		return "00" + new BigInteger(1,result).toString(16).toUpperCase();
	}
	
	private static void compress(int[] state, byte[] blocks, int len) {	// RIPEMD160 암호화 진행에 필요한 압축 메소드
		if (len % BLOCK_LEN != 0)
